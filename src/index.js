...
function buildGridKeyboard(totalItems, pageIndex, pageSize, mediaType, lang, mediaId, activeIndex, cols = GRID_COLS) {
  ...
  for (let i = start; i < end; i++) {
    ...
    row.push({ text: label, callback_data: `pick:${mediaType}:${lang}:${i}:${mediaId}` });
  ...
  if (pageIndex > 0) {
    navRow.push({ text: '◀ Prev', callback_data: `page:${mediaType}:${lang}:${pageIndex - 1}:${mediaId}` });
  }
  
  navRow.push({ text: `Page ${pageIndex + 1}/${totalPages}`, callback_data: 'noop' });
  
  if (pageIndex < totalPages - 1) {
    navRow.push({ text: 'Next ▶', callback_data: `page:${mediaType}:${lang}:${pageIndex + 1}:${mediaId}` });
  }
  
  rows.push(navRow);

  const actionRow = [
    { text: '📩 Send To Me', callback_data: `send:${mediaType}:${lang}:${activeIndex}:${mediaId}` },
    { text: '🌐 Languages', callback_data: `langs:${mediaType}:${mediaId}` },
    { text: '❌ Close', callback_data: `back:main:${mediaId}` }
  ];
...
async function sendMainMenu(bot, chatId, data, messageId = null) {
  ...
  if (messageId) {
    await bot.editMessageText(chatId, messageId, caption, { parse_mode: PARSE_MODE, reply_markup: keyboard, link_preview_options });
  } else {
    await bot.sendMessage(chatId, caption, { parse_mode: PARSE_MODE, reply_markup: keyboard, link_preview_options });
  }
}

async function sendResultsGrid(bot, chatId, data, mediaType, lang, pageIndex, messageId, activeIndex = -1) {
  const imageList = (data.images?.[mediaType]?.[lang]) || [];
  const total = imageList.length;

  if (total === 0) {
    return bot.editMessageText(chatId, messageId, `<b>${escapeHtml(data.title)}</b>\n\n<i>No ${escapeHtml(mediaType)} found for this language.</i>`, { parse_mode: PARSE_MODE, reply_markup: { inline_keyboard: [[{ text: '🌐 Change Language', callback_data: `langs:${mediaType}:${data.media_id}` }, { text: '« Back', callback_data: `back:main:${data.media_id}` }]] } });
  }
  ...
  const caption = `<b>${escapeHtml(data.title)}</b> — <i>${escapeHtml(mediaType)} (${lang.toUpperCase()})</i>\nPreviewing image ${activeIndex + 1}/${total}. Select a number to change the preview.`;
  const keyboard = buildGridKeyboard(total, page, GRID_PAGE_SIZE, mediaType, lang, data.media_id, activeIndex);
...
async function sendDetailsPage(bot, chatId, data, messageId) {
  ...
  await bot.editMessageText(chatId, messageId, text, { parse_mode: PARSE_MODE, reply_markup: keyboard, link_preview_options });
}
...
async function sendLanguageSelection(bot, message, mediaData, mediaType) {
  ...
  const buttons = langs.sort().map(langKey => {
    ...
    return [{ text: label, callback_data: `view:${mediaType}:${langKey}:0:${mediaData.media_id}` }];
  });
  
  buttons.push([{ text: '« Back to Grid', callback_data: `view:${mediaType}:en:0:${mediaData.media_id}` }]);
...
async function handleMessage(bot, message, env) {
  ...
  try {
    if (text.startsWith('/')) {
      ...
      if (command === '/start') return bot.sendMessage(chatId, `<b>Welcome to PosterFlix</b>\nSend a movie or TV show name to begin.\n\n<i>Example:</i> <code>The Matrix 1999</code>`, { parse_mode: PARSE_MODE });
      if (command === '/about') return bot.sendMessage(chatId, ABOUT_TEXT, { parse_mode: PARSE_MODE });
      if (command === '/faq') return bot.sendMessage(chatId, FAQ_TEXT, { parse_mode: PARSE_MODE });
      if (command === '/disclaimer') return bot.sendMessage(chatId, DISCLAIMER_TEXT, { parse_mode: PARSE_MODE });
    }

    if (!text) return;

    if (!validateYearInQuery(text)) {
      await bot.sendMessage(chatId, `<b>⚠️ Tip</b> — For the most accurate results, include the year in your search.\n<i>Example:</i> <code>Inception 2010</code>`, { parse_mode: PARSE_MODE });
    }
    ...
    if (!response.ok) {
      return bot.sendMessage(chatId, `<b>😕 No results</b> — Couldn't find results for <code>${escapeHtml(text)}</code>. Please check spelling or add a year.`, { parse_mode: PARSE_MODE });
    }

    const data = await response.json();
    if (!data || !data.media_id) {
      return bot.sendMessage(chatId, `<b>🔍 No usable result</b> — The search completed but returned incomplete data. Try a different query.`, { parse_mode: PARSE_MODE });
    }

    await env.BOT_STATE.put(getMediaCacheKey(data.media_id), JSON.stringify(data), { expirationTtl: CACHE_TTL });
    await sendMainMenu(bot, chatId, data);

  } catch (err) {
    console.error('handleMessage error', err);
    await bot.sendMessage(chatId, `<b>⚠️ Error</b> — Could not perform search: <code>${escapeHtml(String(err.message || err))}</code>`, { parse_mode: PARSE_MODE });
  }
}
